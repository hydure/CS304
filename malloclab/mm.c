...
team_t team={
    "Colin Lightfoot",                                    /* team name */
    "Colin Lightfoot",                       /* first member full name */
    "CFLightfoot@email.wm.edu",           /* first member email address*/
...
*/
#define UNSCALED_POINTER_ADD(p,x) ((char*)(p) + (x))
#define UNSCALED_POINTER_SUB(p,x) ((char*)(p) - (x))
...
void* mm_malloc (size_t size) {
  ...
  // Added one word for the initial size header
  size += WORD_SIZE;
  if (size <= MIN_BLOCK_SIZE) {
    // Make sure we allocate enough space for a blockInfo
  ...
  while(ptrFreeBlock == NULL)
  {
      ...
      //remove this node from list and manipulate it
      ptrFreeBlock = searchFreeList(reqSize);
  }
  ...
  if((blockSize - reqSize) < MIN_BLOCK_SIZE)
  {
      ...
      {
          nextBlock = (BlockInfo *)(UNSCALED_POINTER_ADD(
                      nextBlock,SIZE(nextBlock->sizeAndTags)-WORD_SIZE));
          nextBlock->sizeAndTags |= TAG_PRECEDING_USED;
      }
  }
  else
  {
      ...
      *((size_t *)UNSCALED_POINTER_ADD(
                              nextBlock,SIZE(tags)-WORD_SIZE)) = tags;
...
void mm_free (void *ptr) {
  ...
  followingBlock = (BlockInfo *)UNSCALED_POINTER_ADD(
                                        blockInfo, SIZE(blockTags));
  *((size_t *)UNSCALED_POINTER_ADD(blockInfo,SIZE(blockTags)-WORD_SIZE)) =
                                                      blockTags;
  followingBlock->sizeAndTags &= (~TAG_PRECEDING_USED);
 
  // join the former block with the rest of the free areas
...
/* reallocates block referenced by the ptr's memory in order to increase its size */
void* mm_realloc(void* ptr,size_t size)
{
  BlockInfo *currentBlock = NULL;
  size_t currentSize;
  
  // if size is <= 0, we free the block and return NULL
  if(size <= 0){
    mm_free(ptr);
    return NULL;
  }
  // if the ptr is NULL, then we call mm_malloc
  if(!ptr){
  ...
  // if realloc fails, we leave everything as is
  if(!newptr){
    return 0;
  }
  // copies the old data
  currentBlock = (BlockInfo *)UNSCALED_POINTER_SUB(ptr, WORD_SIZE);
